...
def speech_to_text_with_timestamps(audio_file_path, language_code="ko-KR"):
    ...
    try:
        ...
        config = speech.RecognitionConfig(
            ...
            # model="phone_call",  # 전화 통화 특화 (분당 21원)
            # model="video",       # 영상 오디오 특화 (분당 21원)
            # model="medical",     # 의료 용어 특화 (분당 21원)
        )
        ...
        try:
            ...
            if "Sync input too long" in str(sync_error):
                ...
                response = operation.result(timeout=120)  # 최대 2분 대기
            else:
                raise sync_error
    ...
    for i, res in enumerate(result["results"]):
        ...
        for word_data in res["words"]:
            ...
            formatted_results.append(
                f"  [{start_time:.2f}s - {end_time:.2f}s] '{word}' (신뢰도: {confidence:.2f})"
            )
