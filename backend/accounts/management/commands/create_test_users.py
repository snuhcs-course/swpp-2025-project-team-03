...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if not Account.objects.filter(email=teacher_email).exists():
            ...
            self.stdout.write(
                self.style.SUCCESS(f'✓ 선생님 계정 생성: {teacher_email} / teacher123')
            )
        else:
            self.stdout.write(
                self.style.WARNING(f'선생님 계정이 이미 존재합니다: {teacher_email}')
            )
        ...
        if not Account.objects.filter(email=student_email).exists():
            ...
            self.stdout.write(
                self.style.SUCCESS(f'✓ 학생 계정 생성: {student_email} / student123')
            )
        else:
            self.stdout.write(
                self.style.WARNING(f'학생 계정이 이미 존재합니다: {student_email}')
            )
