...
import pytest
from reports.utils.analyze_achievement import ACHIEVEMENT_CODE_NOT_FOUND, parse_curriculum
...
class TestAnalyzeAchievementEdgeCases:
    ...
    @patch("reports.utils.analyze_achievement.logger")
    @patch("reports.utils.analyze_achievement.calculate_statistics")
    @patch("reports.utils.analyze_achievement.Question.objects")
    def test_parse_curriculum_no_school_level(self, mock_question_objects, mock_calc_stats, mock_logger):
        ...
        assert any("Could not determine school level" in str(call) for call in mock_logger.warning.call_args_list)
    ...
    def test_parse_curriculum_no_relevant_standards(
        self, mock_question_objects, mock_open, mock_csv_reader, mock_calc_stats, mock_logger
    ):
        ...
        result = parse_curriculum(student_id=1, class_id=1)
        # Warning 메시지가 출력되었는지 확인
        assert any("No achievement standards found" in str(call) for call in mock_logger.warning.call_args_list)
    ...
    def test_parse_curriculum_no_best_code(
        ...
        mock_question_qs.__iter__ = Mock(return_value=iter(mock_questions))
        mock_question_qs.select_related.return_value = mock_question_qs
        mock_question_qs.filter.return_value = mock_question_qs
        mock_question_objects.filter.return_value = mock_question_qs
        ...
        # 성취기준을 찾지 못한 경우 NOT_FOUND로 저장되어야 함
        mock_question.save.assert_called_once()
        assert mock_question.achievement_code == ACHIEVEMENT_CODE_NOT_FOUND
    ...
    @patch("reports.utils.analyze_achievement.calculate_statistics")
    @patch("reports.utils.analyze_achievement.Question.objects")
    def test_parse_curriculum_exception_in_question_processing(
        self, mock_question_objects, mock_calc_stats, mock_logger
    ):
        """질문 처리 중 예외 발생 테스트"""
        mock_question = Mock()
        mock_question.id = 1
        ...
        result = parse_curriculum(student_id=1, class_id=1)
        # 에러 메시지가 출력되었는지 확인
        assert any("Error processing question" in str(call) for call in mock_logger.error.call_args_list)
